; $(function() {
    ...
    $('#js-call-native-link').on('singletap', function() {
        axemas.call('open-sidebar-from-native', {}, function (data) {
            axemas.dialog("FATTO, Risposta: " + JSON.stringify(data));
        });
    });
    ...
    axemas.register("display-device-model", function(payload, cb){
        axemas.dialog('Native response', "Some information on the device:\n- "
                        + payload.name + "\n- " + payload.other, ['Close']);
        cb({ message: "message from JS in callback!" });
    });
